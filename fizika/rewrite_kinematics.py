...
class SVGBuilder:
    ...
    def arc(self, p, r, start_ang, end_ang, label=""):
        ...
        path = f'<path d="M {x1} {y1} A {r} {r} 0 {large_arc} {sweep} {x2} {y2}" fill="none" stroke="black" marker-end="url(#arrow)"/>'
        self.add(path, [(ps[0]-r, ps[1]-r), (ps[0]+r, ps[1]+r)])
        if label:
            mx = ps[0] + (r+15) * math.cos((sa+ea)/2)
            my = ps[1] + (r+15) * math.sin((sa+ea)/2)
            self.add(f'<text x="{mx-10}" y="{my+5}" font-style="italic">{label}</text>')
...
def draw_mech(svg, show_vel=False, show_acc=False):
    ...
    svg.arc(O, 30, 0, alfa, "&alpha;")
    A_ext = add(A, mult(sub(O, A), 0.3))
    svg.line(A, A_ext, dash="4,4")
    svg.arc(A, 30, alfa+180, alfa+180+beta, "&beta;")
    B_ext = add(B, mult(sub(A, B), 0.3))
    svg.line(B, B_ext, dash="4,4")
    svg.arc(B, 30, theta_AD+180, theta_AD+180+gamma, "&gamma;")
    svg.arc(O, 40, alfa, alfa+50 if W1>0 else alfa-50, "&omega;&#8321;")
    
    # Missing angles from user image 3
    svg.line(O2, (O2[0]+0.3, O2[1]), dash="4,4")
    svg.arc(O2, 40, 180, 180+delta, "&delta;")
    svg.line(D, (D[0]+0.3, D[1]), dash="4,4")
    svg.arc(D, 50, 0, phi, "&phi;")
    ...
    if show_vel:
        ...
        svg.arc(A, 40, theta_AD, theta_AD+(40 if w2>0 else -40), "&omega;&#8322;")
        svg.arc(B, 40, theta_BE, theta_BE+(40 if w3>0 else -40), "&omega;&#8323;")
        svg.arc(E, 40, delta+180, delta+180+(40 if w4>0 else -40), "&omega;&#8324;")
    ...
    if show_acc:
        ...
        svg.arc(A, 50, theta_AD, theta_AD+(40 if eps2>0 else -40), "&varepsilon;&#8322;")
        svg.arc(B, 50, theta_BE, theta_BE+(40 if eps3>0 else -40), "&varepsilon;&#8323;")
        svg.arc(E, 50, delta+180, delta+180+(40 if eps4>0 else -40), "&varepsilon;&#8324;")
...
poly_D = build_polygon("D", [(vA, "V", "A", (10, -10)), (vDA, "V", "DA", (-25, -15))], (vD, "V", "D", (10, 15)), scale=100, title="Taško D greičio planas")
poly_B = build_polygon("B", [(vA, "V", "A", (10, -10)), (vBA, "V", "BA", (-25, -15))], (vB, "V", "B", (5, 15)), scale=100, title="Taško B greičio planas")
poly_E = build_polygon("E", [(vB, "V", "B", (10, 15)), (vEB, "V", "EB", (5, -20))], (vE, "V", "E", (-25, -5)), scale=100, title="Taško E greičio planas")

poly_aD = build_polygon("D", [(aA, "a", "A", (-20, 10)), (aDAn, "a", "DA<tspan dy='-5'>n</tspan>", (5, -20)), (aDAt, "a", "DA<tspan dy='-5'>&tau;</tspan>", (5, 10))], (aD, "a", "D", (-25, -15)), scale=20, title="Taško D pagreičio planas")
poly_aB = build_polygon("B", [(aA, "a", "A", (-20, 10)), (aBAn, "a", "BA<tspan dy='-5'>n</tspan>", (5, -20)), (aBAt, "a", "BA<tspan dy='-5'>&tau;</tspan>", (5, 10))], (aB, "a", "B", (-25, -15)), scale=20, title="Taško B pagreičio planas")
poly_aE = build_polygon("E", [(aB, "a", "B", (-25, -15)), (aEBn, "a", "EB<tspan dy='-5'>n</tspan>", (5, 15)), (aEBt, "a", "EB<tspan dy='-5'>&tau;</tspan>", (5, -15))], (aE, "a", "E", (10, -20)), scale=20, title="Taško E pagreičio planas")
...
<p class="eq">x: V<sub>D</sub> &middot; cos({fmt(phi)}&deg;) = V<sub>A</sub> &middot; cos({fmt(theta_VA)}&deg;) + V<sub>DA</sub> &middot; cos({fmt(theta_VDA_base)}&deg;)</p>
<p class="eq">y: V<sub>D</sub> &middot; sin({fmt(phi)}&deg;) = V<sub>A</sub> &middot; sin({fmt(theta_VA)}&deg;) + V<sub>DA</sub> &middot; sin({fmt(theta_VDA_base)}&deg;)</p>
<p>Įstatome skaitines reikšmes:</p>
<p class="eq">x: V<sub>D</sub> &middot; ({fmt(math.cos(rad(phi)))}) = {fmt(vA_mag)} &middot; ({fmt(math.cos(rad(theta_VA)))}) + V<sub>DA</sub> &middot; ({fmt(math.cos(rad(theta_VDA_base)))})</p>
<p class="eq">y: V<sub>D</sub> &middot; ({fmt(math.sin(rad(phi)))}) = {fmt(vA_mag)} &middot; ({fmt(math.sin(rad(theta_VA)))}) + V<sub>DA</sub> &middot; ({fmt(math.sin(rad(theta_VDA_base)))})</p>
...
<p>Projekcijų į x ir y ašis lygtys:</p>
<p class="eq">x: V<sub>E</sub> &middot; cos({fmt(theta_VE_base)}&deg;) = V<sub>B,x</sub> + V<sub>EB</sub> &middot; cos({fmt(theta_VEB_base)}&deg;)</p>
<p class="eq">y: V<sub>E</sub> &middot; sin({fmt(theta_VE_base)}&deg;) = V<sub>B,y</sub> + V<sub>EB</sub> &middot; sin({fmt(theta_VEB_base)}&deg;)</p>
<p>Įstatome skaitines reikšmes (kur V<sub>B,x</sub>={fmt(vB[0])}, V<sub>B,y</sub>={fmt(vB[1])}):</p>
<p class="eq">x: V<sub>E</sub> &middot; ({fmt(math.cos(rad(theta_VE_base)))}) = {fmt(vB[0])} + V<sub>EB</sub> &middot; ({fmt(math.cos(rad(theta_VEB_base)))})</p>
<p class="eq">y: V<sub>E</sub> &middot; ({fmt(math.sin(rad(theta_VE_base)))}) = {fmt(vB[1])} + V<sub>EB</sub> &middot; ({fmt(math.sin(rad(theta_VEB_base)))})</p>
<p>Išsprendę gauname:</p>
...
<p>Projekcijų į x ir y ašis lygtys (kur b<sub>x</sub>, b<sub>y</sub> yra a<sub>A</sub> + a<sub>DA</sub><sup>n</sup> suma):</p>
<p class="eq">x: a<sub>D</sub> &middot; cos({fmt(phi)}&deg;) = {fmt(b_vec_a[0])} + a<sub>DA</sub><sup>&tau;</sup> &middot; cos({fmt(theta_aDAt_base)}&deg;)</p>
<p class="eq">y: a<sub>D</sub> &middot; sin({fmt(phi)}&deg;) = {fmt(b_vec_a[1])} + a<sub>DA</sub><sup>&tau;</sup> &middot; sin({fmt(theta_aDAt_base)}&deg;)</p>
<p>Įstatome skaitines reikšmes:</p>
<p class="eq">x: a<sub>D</sub> &middot; ({fmt(math.cos(rad(phi)))}) = {fmt(b_vec_a[0])} + a<sub>DA</sub><sup>&tau;</sup> &middot; ({fmt(math.cos(rad(theta_aDAt_base)))})</p>
<p class="eq">y: a<sub>D</sub> &middot; ({fmt(math.sin(rad(phi)))}) = {fmt(b_vec_a[1])} + a<sub>DA</sub><sup>&tau;</sup> &middot; ({fmt(math.sin(rad(theta_aDAt_base)))})</p>
...
<p>Projekcijų į x ir y ašis lygtys (kur b<sub>x</sub>, b<sub>y</sub> yra a<sub>B</sub> + a<sub>EB</sub><sup>n</sup> - a<sub>E</sub><sup>n</sup> suma):</p>
<p class="eq">x: a<sub>E</sub><sup>&tau;</sup> &middot; cos({fmt(theta_aEt_base)}&deg;) = {fmt(b_vec_a2[0])} + a<sub>EB</sub><sup>&tau;</sup> &middot; cos({fmt(theta_aEBt_base)}&deg;)</p>
<p class="eq">y: a<sub>E</sub><sup>&tau;</sup> &middot; sin({fmt(theta_aEt_base)}&deg;) = {fmt(b_vec_a2[1])} + a<sub>EB</sub><sup>&tau;</sup> &middot; sin({fmt(theta_aEBt_base)}&deg;)</p>
<p>Įstatome skaitines reikšmes:</p>
<p class="eq">x: a<sub>E</sub><sup>&tau;</sup> &middot; ({fmt(math.cos(rad(theta_aEt_base)))}) = {fmt(b_vec_a2[0])} + a<sub>EB</sub><sup>&tau;</sup> &middot; ({fmt(math.cos(rad(theta_aEBt_base)))})</p>
<p class="eq">y: a<sub>E</sub><sup>&tau;</sup> &middot; ({fmt(math.sin(rad(theta_aEt_base)))}) = {fmt(b_vec_a2[1])} + a<sub>EB</sub><sup>&tau;</sup> &middot; ({fmt(math.sin(rad(theta_aEBt_base)))})</p>
<p>Sprendžiant lygčių sistemą randame:</p>
...
with open('/Users/ugniusvaitiekenas/srotas-ai-agent/fizika/generate_kinematics_html_v2.py', 'w', encoding='utf-8') as f:
    f.write(html)
